...
object LDEventBus {
     ...
     *
     * @param key 事件 key
     * @param value 事件值 默认是 0
     * @param delay 事件延迟发送事件 毫秒值
     */
    fun post(key: String, value: Any = 0, delay: Long = 0) {
        if (delay == 0L) {
     ...
     * @param owner 生命周期所有者
     * @param key 订阅事件 key
     * @param clazz 订阅事件类型 配合 post 方法，这里如果没有特殊需求就默认为 Int
     ...
     * @param value 事件值
     */
    fun postOrderly(key: String, value: Any = 0) {
