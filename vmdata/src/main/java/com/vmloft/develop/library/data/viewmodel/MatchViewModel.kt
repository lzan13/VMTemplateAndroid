...
import com.vmloft.develop.library.data.repository.MatchRepository
...
class MatchViewModel(private val repo: MatchRepository, private val infoRepo: InfoRepository) : BViewModel() {
     ...
     * 获取匹配列表
     */
    fun matchList(gender: Int = 2, type: Int = -1, page: Int = CConstants.defaultPage, limit: Int = CConstants.defaultLimit) {
        viewModelScope.launch(Dispatchers.Main) {
    ...
    fun randomMatch(gender: Int = -1, type: Int = 0) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = withContext(Dispatchers.IO) {
                Thread.sleep(CUtils.random(2,3) * CConstants.timeSecond)
