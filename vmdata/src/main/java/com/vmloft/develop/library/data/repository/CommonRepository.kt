...
class CommonRepository : BaseRepository() {
    ...
    suspend fun download(attachment: Attachment): ResponseBody {
        ...
        return APIRequest.commonAPI.download(url)
    }

    /**
     * 获取客户端配置，这里控制超过 1 小时去服务器请求
     */
    suspend fun appConfig(): RResult<Config> {
        val time = DSPManager.getAppConfigTime()
        val intervalTime = System.currentTimeMillis() - time
        // 这里一般使用缓存，只有超过一定时间才去服务器获取，设置为 1 天，TODO 测试情况为 1 分钟
        if (SignManager.getSignId().isNotEmpty() && intervalTime < if (CSPManager.isDebug()) CConstants.timeMinute else CConstants.timeDay * 1) {
            val config = AppDatabase.getInstance().configDao().query("appConfig")
            if (config != null) {
                return RResult.Success("", config)
            }
        }
        val result = safeRequest { executeResponse(APIRequest.commonAPI.appConfig()) }
        if (SignManager.getSignId().isNotEmpty() && result is RResult.Success) {
            DSPManager.setAppConfigTime(System.currentTimeMillis())
            // 保存数据
            result.data?.let { AppDatabase.getInstance().configDao().insert(it) }
        }
        return result
    }
     ...
     */
    suspend fun checkVersion(): RResult<Version> {
        ...
        if (result is RResult.Success) {
            ...
            // 保存数据
            result.data?.let { AppDatabase.getInstance().versionDao().insert(it) }
        }
     ...
     * 获取分类集合
     */
    suspend fun categoryList(): RResult<RPaging<Category>> {
        // 根据参数优先从本地获取，这里检查下缓存时间，大于 1 天就重新从服务器获取
        val time = DSPManager.getCategoryTime()
        val intervalTime = System.currentTimeMillis() - time
        // 这里一般使用缓存，只有超过一定时间才去服务器获取，设置为 7 天，TODO 测试情况为 1 分钟
        if (SignManager.getSignId().isNotEmpty() && intervalTime < if (CSPManager.isDebug()) CConstants.timeMinute else CConstants.timeDay * 7) {
        ...
        if (result is RResult.Success) {
            ...
            // 先清空原来的数据
            AppDatabase.getInstance().categoryDao().clear()
            // 重新插入
     ...
     * 获取职业集合
     */
    suspend fun professionList(): RResult<RPaging<Profession>> {
        // 根据参数优先从本地获取，这里检查下缓存时间，大于 1 天就重新从服务器获取
        // 根据参数优先从本地获取
        ...
        if (result is RResult.Success) {
            ...
            // 先清空原来的数据
            AppDatabase.getInstance().professionDao().clear()
            // 重新插入
