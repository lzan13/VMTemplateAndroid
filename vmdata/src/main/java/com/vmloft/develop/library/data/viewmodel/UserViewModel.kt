...
class UserViewModel(
    ...
    private val relationRepo: RelationRepository,
...
//            val part: MultipartBody.Part = MultipartBody.Part.createFormData("cover", file.name, body)
     ...
     * 获取用户信息
     */
    fun userList(ids: List<String>) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = repo.getUserList(ids)

            if (result is RResult.Success && result.data != null) {
                // 将用户信息加入到缓存
                CacheManager.putUsers(result.data!!)
                emitUIState(data = result.data, type = "userList")
                return@launch
            } else if (result is RResult.Error) {
                emitUIState(isSuccess = false, code = result.code, error = result.error)
            }
    ...
    fun follow(id: String) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = relationRepo.follow(id)
            if (result is RResult.Success) {
    ...
    fun cancelFollow(id: String) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = relationRepo.cancel(id)
            if (result is RResult.Success) {
     ...
     * 关注列表
     */
    fun relationList(type: Int, page: Int = CConstants.defaultPage, limit: Int = CConstants.defaultLimit) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = relationRepo.relationList(type, page, limit)
            if (result is RResult.Success) {
                emitUIState(data = result.data, type = "relationList")
                return@launch
            } else if (result is RResult.Error) {
                emitUIState(isSuccess = false, code = result.code, error = result.error)
            }
     ...
     * 视频奖励
     */
    fun videoReward(params: Map<String, String>) {
        viewModelScope.launch(Dispatchers.Main) {
            emitUIState(true)
            val result = tradeRepo.videoReward(JsonUtils.map2json(params).toRequestBody("application/json".toMediaType()))
            if (result is RResult.Success) {
                emitUIState(type = "videoReward")
                return@launch
            } else if (result is RResult.Error) {
                emitUIState(isSuccess = false, code = result.code, error = result.error)
            }
