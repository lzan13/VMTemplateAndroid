...
object IMChatManager {
    ...
    fun saveMessage(message: EMMessage) {
        ...
        // 保存会话时间
        setConversationTime(getConversation(message.conversationId(), message.chatType.ordinal), message.localTime())
    ...
    fun sendMessage(
        ...
        message.setMessageStatusCallback(object : EMCallBack {
            override fun onSuccess() {
                VMLog.i("消息发送成功 msgId ${message.msgId} - $message")
                LDEventBus.post(IMConstants.Common.updateMsgEvent, message)
                success.invoke()
            }
            ...
            override fun onError(code: Int, desc: String) {
                ...
                if (message.type != EMMessage.Type.CMD && code == 210) {
                    saveSysMessage(message.conversationId(), "对方已将你加入黑名单，无法发送消息")
                }
                LDEventBus.post(IMConstants.Common.updateMsgEvent, message)
...
//                LDEventBus.post(IMConstants.Common.updateMsgEvent, message)
     ...
     * 获取消息类型
     */
    fun getMsgType(message: EMMessage): Int {
        var extType = message.getIntAttribute(IMConstants.Common.msgAttrExtType, IMConstants.MsgType.imUnknown)
        return if (extType == IMConstants.MsgType.imSystem || extType == IMConstants.MsgType.imRecall) {
            ...
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imGiftReceive else IMConstants.MsgType.imGiftSend
        } else if (extType == IMConstants.MsgType.imCall) {// 通话
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imCallReceive else IMConstants.MsgType.imCallSend
        } else if (extType == IMConstants.MsgType.imBigEmotion) {// 大表情
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imBigEmotionReceive else IMConstants.MsgType.imBigEmotionSend
        } else if (message.type == EMMessage.Type.TXT) {// 文本
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imTextReceive else IMConstants.MsgType.imTextSend
        } else if (message.type == EMMessage.Type.IMAGE) {// 图片
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imPictureReceive else IMConstants.MsgType.imPictureSend
        } else if (message.type == EMMessage.Type.VIDEO) {// 视频
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imVideoReceive else IMConstants.MsgType.imVideoSend
        } else if (message.type == EMMessage.Type.LOCATION) {// 位置
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imLocationReceive else IMConstants.MsgType.imLocationSend
        } else if (message.type == EMMessage.Type.VOICE) {// 语音
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imVoiceReceive else IMConstants.MsgType.imVoiceSend
        } else if (message.type == EMMessage.Type.FILE) {// 文件
            if (message.direct() == EMMessage.Direct.RECEIVE) IMConstants.MsgType.imFileReceive else IMConstants.MsgType.imFileSend
        } else {// 未知，显示提示文本
     ...
     * 获取消息摘要信息
     */
    fun getSummary(message: EMMessage?): String {
        if (message == null) return VMStr.byRes(R.string.im_empty)

        var content = ""
        val type: Int = getMsgType(message)
        if (type == IMConstants.MsgType.imSystem) {// 系统提醒
            content = message.getStringAttribute(IMConstants.Common.msgAttrSystem, "")
            if (content.isEmpty()) {
            ...
            content = "[" + VMStr.byRes(R.string.im_call) + " - " + (message.body as EMTextMessageBody).message + "]"
     ...
     * 是否要显示时间戳
     */
    fun isShowTime(position: Int, message: EMMessage): Boolean {
        if (position == 0) {
            return true
        }
        val list = getCacheMessages(message.conversationId(), message.chatType.ordinal)
        val preMsg = list[position - 1]
        return message.localTime() - preMsg.localTime() > CConstants.timeMinute * 5
    }
}